...
function _M:Connect(handler, handler_self)
    ...
    table.insert(self.handlers, {handler, handler_self})
    local t = { }
    t.Disconnect = function()
        return self:Disconnect(handler, handler_self)
    end
...
function _M:Disconnect(handler, handler_self)
    assert(self ~= nil and type(self) == "table", "Invalid Event (make sure you're using ':' not '.')")
